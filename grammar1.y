...
assignment_expression
	...
	: var_expression assignment_operator logical_or_expression 
		{
		//STOPPED HERE
		if(check_typ_id(csym,$1.idVal)==-1){yyerror();}
		if ($2 == 1){
			$$.node_ptr = create_node("=", $1.node_ptr, $3.node_ptr);
			printf("=: %p",$$.node_ptr);
		}
...
initializer_list
	: initializer {if(assign_id(csym,idc,$1.fVal,1,t,x,y++)==-1){yyerror();}}
	| initializer_list ',' initializer {if(assign_id(csym,idc,$3.fVal,1,t,x,y++)==-1){yyerror();}}
	;

list_of_strings
	: T_STRING_LITERAL {if(assign_string(csym,idc,$1,x)!=2){yyerror();}x++;}
	| list_of_strings ',' T_STRING_LITERAL {if(assign_string(csym,idc,$3,x)!=2){yyerror();}x++;}
	;	
