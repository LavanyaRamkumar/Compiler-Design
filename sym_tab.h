...
{
	int i=0,j=0,k=0;
	printf("DATA STORED : id\tname\ttype\tln no.\tused at\n");
	for(i=0;i<cur;i++)
	{
		printf("ST %d:\n",i);
		printf("parent: %d\n",symtab[i].parent);
...
{
	int i=0,j=0,k=0;
	printf("ST\tparent\tid\tname\ttype\tln no.\tused at %55s val\n"," ");
	for(i=0;i<cur;i++)
	{
		//printf("ST %d:\n",i);
		//printf("parent: %d\n",symtab[i].parent);
		...
		{	int le = strlen(symtab[i].vars[j].type);
			...
			printf("%d\t%d\t%d\t%s\t%s\t%d\t",i,symtab[i].parent,symtab[i].vars[j].id,symtab[i].vars[j].name,symtab[i].vars[j].type,symtab[i].vars[j].line_no);
			k=0;
			printf("[");
			while(symtab[i].vars[j].ln_used[k]!=-1)
			{
			...
			if (strcmp(type,"int")==0) {
				...
				if(symtab[i].vars[j].length==0 && symtab[i].vars[j].breadth==0){
					printf("%d ",(*(int*)symtab[i].vars[j].ptr));
				}
				else if (symtab[i].vars[j].breadth==0)
				{
					for(int p=0;p<symtab[i].vars[j].length;p++)
						printf("%d ",((int *)symtab[i].vars[j].ptr)[p]);
				}
				else
				{
					for(int p=0;p<symtab[i].vars[j].length;p++){
						for(int q=0;q<symtab[i].vars[j].breadth;q++){
							printf("%d ",((int *)symtab[i].vars[j].ptr)[p*symtab[i].vars[j].breadth+q]);
						}
						printf("\t");	
					}
				}
			...
			if (strcmp(type,"float")==0) {
				if(symtab[i].vars[j].length==0 && symtab[i].vars[j].breadth==0){
					printf("%f ",(*(float*)symtab[i].vars[j].ptr));
				}
				else if (symtab[i].vars[j].breadth==0)
				{
					for(int p=0;p<symtab[i].vars[j].length;p++)
						printf("%f ",((float *)symtab[i].vars[j].ptr)[p]);
				}
				else
				{
					for(int p=0;p<symtab[i].vars[j].length;p++){
						for(int q=0;q<symtab[i].vars[j].breadth;q++){
							printf("%f ",((float *)symtab[i].vars[j].ptr)[p*symtab[i].vars[j].breadth+q]);
						}
						printf("\t");	
					}
				}
			...
			if (strcmp(type,"char")==0) {
				if(symtab[i].vars[j].length==0 && symtab[i].vars[j].breadth==0){
					printf("%c ",(*(char*)symtab[i].vars[j].ptr));
				}
				else if (symtab[i].vars[j].breadth==0)
				{
					for(int p=0;p<symtab[i].vars[j].length;p++)
						printf("%c ",((char *)symtab[i].vars[j].ptr)[p]);
				}
				else
				{
					for(int p=0;p<symtab[i].vars[j].length;p++){
						for(int q=0;q<symtab[i].vars[j].breadth;q++){
							printf("%c",((char *)symtab[i].vars[j].ptr)[p*symtab[i].vars[j].breadth+q]);
						}
						printf("\t");	
					}
				}
